...
- **Node run:** through the same `specialistOptions`/dept-resolution path as `hand_off` and `@mention` — the Phase 1 capability-parity invariant extends to graph nodes by construction. Loop/verify rounds implemented in the engine (porting today's executor round logic). Wall time: `AIOS_JOB_WALL_TIME_MS` unchanged, measured from the goal's last status transition (`updated_at`) — a budget-paused goal resumed next day gets a fresh window instead of instant expiry.
